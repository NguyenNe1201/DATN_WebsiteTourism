                        ...
                        <div class="row mrg-0 ">
                            <div class="tr-single-box">
                                <div class="tr-single-header">
                                    <h3 class="dashboard-title">Danh Sách Lịch Sử Đặt Tour</h3>
                                </div>
                                <div class="tr-single-body">
                                    <div class="table-responsive" style="margin-top: 20px; ">
                                        <table class="table table-striped table-hover" id="table_tourbook_history"
                                            data-page-length='5'>
                                            <thead>
                                                <tr>
                                                    <th>STT</th>
                                                    <th style="max-width:240px;">Tour</th>
                                                    <th>Thông tin</th>
                                                    <th>Thanh toán</th>
                                                    <th>Trạng thái</th>
                                                    <th>Function</th>
                                            ...
                                            <tbody>
                                                <!-- tr -->
                                                @if (!empty($getSignUpTour) && $getSignUpTour->count() > 0)
                                                    @foreach ($getSignUpTour as $key => $item)
                                                        <tr>
                                                            <td>{{ $key + 1 }}</td>
                                                            <td>
                                                                <b>{{ $item->tour->tourname }}</b> <br>
                                                                <i class=" ti-location-pin"></i>
                                                                {{ $item->tour_location->location_name }} <br>
                                                                <span style="font-weight: 600; margin-right: 5px;">Ngày đăng
                                                                    kí:</span>{{ $item->Regis_date->format('d/m/Y') }}<br>
                                                                <span style="font-weight: 600; margin-right: 5px;">Ngày khởi
                                                                    hành:</span>{{ $item->start_date->format('d/m/Y') }}<br>
                                                                <span style="font-weight: 600; margin-right: 5px;">Ngày
                                                                    về:</span>{{ $item->end_date->format('d/m/Y') }}<br>
                                                            </td>
                                                            <td>
                                                                <span style="font-weight: 600; margin-right: 5px;"> Số
                                                                    người:</span>{{ $item->Regis_number }} <br>
                                                                ...
                                                                <span style="font-weight: 600; margin-right: 5px;">Ghi
                                                                    chú:</span>{{ $item->note_tour }} <br>
                                                                @if (!empty($item->guider_tour))
                                                                    <b>Guider:</b> <span style="color: #ff9800;font-weight: 600;">{{ $item->guider_tour->name_guider }}</span> <br>
                                                                @endif
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td style="text-align: center;">
                                                                <div class="dropdown dropdown-action">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-right"
                                                                        ...
                                                                        @if ($item->status == 1)
                                                                            <form
                                                                                action="{{ route('Up_StatusBookTour_Cancel') }}"
                                                                                ...
                                                                                <button type="submit"
                                                                                    class="push-tour dropdown-item fw-500"><i
                                                                                        class="fa fa-trash-o m-r-5"></i>
                                                                                    Yêu cầu hủy</button>
                                                                                @csrf
                                                                            </form>
                                                                        @endif
                                                                        @if ($item->status == 3 && !$item->reviews()->exists())
                                                                            <a data-target="#review_modal"
                                                                                data-toggle="modal"
                                                                                data-tourid="{{ $item->tour_id }}"
                                                                        ...
                                                                        @endif
                                                                        @if ($item->payment->status_payment == 1)
                                                                            <a href="#" class="dropdown-item"><i class="fa fa-fw fa-eye m-r-5"></i> Xem chi tiết</a>
                                                                        @endif
                                                                    </div>
                                                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('post_review_tour') }}" method="POST" role="form"
                        enctype="multipart/form-data" id="review_tour"class="jquery_valiss">
                        <input type="hidden" name="tour_id" id="tour_id" value="">
                        <input type="hidden" name="customer_id_rv" id="customer_id_rv" value="">
                        <input type="hidden" name="sign_up_id" id="signup_id" value="">
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label style="font-size:15px;" for="content" class="col-md-2 col-sm-3">Nội
                                        dung Review:</label>
                                    <div class="col-md-10 col-sm-9">
                                        <textarea name="content_review" id="content_review" class="form-control height-100" required></textarea>
                                    </div>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        $(function(e) {
            $(document).ready(function() {
                var table = $('#table_tourbook_history').DataTable({
                    "pageLength": 10,
                    "lengthMenu": [
                        [5, 10, 15, -1],
                        [5, 10, 15, "All"]
                ...
                $("#review_tour").validate({
                    ...
                    messages: {
                        // rate: "Vui lòng chọn sao đánh giá.",
                        content_review: "Vui lòng nhập nội dung review."
                    }
                });
                // push cancel book tour
                $('.push-tour').on('click', function(event) {
